  ...
  try {
    ...
    const formattedMessages = conversation.messages.map(
      (msg: (typeof conversation.messages)[number]) => ({
        id: msg.id,
        type:
          msg.messageType === "board_member" ? "board_member" : msg.messageType,
        persona: msg.persona,
        name: (msg.metadata as Record<string, unknown> | null | undefined)
          ?.name as string | undefined,
        title: (msg.metadata as Record<string, unknown> | null | undefined)
          ?.title as string | undefined,
        animalSpirit: (
          msg.metadata as Record<string, unknown> | null | undefined
        )?.animalSpirit as string | undefined,
        mantra: (msg.metadata as Record<string, unknown> | null | undefined)
          ?.mantra as string | undefined,
        content: msg.content,
    ...
    const formattedArtifacts = conversation.artifacts.map(
      (artifact: (typeof conversation.artifacts)[number]) => ({
        id: artifact.id,
...
export async function PATCH(
  ...
  try {
    ...
    const { status, phase } = parsed.data;
